...
- SimWar is a SaaS / AI business simulation platform for executive training.
...
- Do not use `gh pr create` when local GitHub CLI is unavailable; push the branch and provide the GitHub PR creation link.
...
For migration/schema PRs, also run:
...
- Implement repository migration in this order:
  1. audit
  2. docs/spec
  3. schema
  4. read mapping
  5. write mapping
  6. parity tests
  7. disposable DB verification
  8. opt-in runtime wiring
...
- Only approved team decisions / canonical settlement inputs can affect settlement.

## Low Token Codex Mode

Prefer short task cards:
...
- Testing: Vitest、contract check、migration check、build gate 已存在。
...
- Vitest / Playwright / Pact：分别保护 unit / service / contract、三端 E2E、consumer-driven contract。
...
docs/
  ...
  devops/               部署、运维、回滚文档
db/
  migrations/           SQL-first PostgreSQL migration 文件，需通过 npm run test:migration
```
...
当前真实包管理器是 npm。以下 npm 命令来自根目录 `package.json`，可以在对应场景中运行：
...
以下是质量工具链路线图推荐的标准命令名，但当前不应假设可运行；除非 `package.json` 或包管理器配置已经补齐，否则将它们视为拟新增命令：
...
pnpm run test:settlement-idempotency
pnpm run test:plugin-boundary
...
- shared contracts / schema / fixtures：`npm run typecheck`、`npm test`、`npm run test:contract`、`npm run test:schema-drift`。
- API routes / repository / settlement：`npm run lint`、`npm run typecheck`、`npm test`、`npm run test:contract`、`npm run test:schema-drift`。
- DB migration / Postgres adapter：`npm run test:migration`；接入真实测试库时运行 `npm run test:migration:apply` 和 `npm run test:postgres-adapter`，并补 adapter contract 或 migration 检查。
...
- 发布前或大改动：运行完整本地质量门禁 `npm run quality`。
...
- 安装或使用项目已声明的本地依赖。
- 运行 `npm run quality`、`npm run format:check`、`npm run lint`、`npm run lint:boundaries`、`npm run check:unused`、`npm run security:audit`、`npm run typecheck`、`npm test`、`npm run test:coverage`、`npm run test:contract`、`npm run test:schema-drift`、`npm run test:migration`、`npm run test:migration:apply`、`npm run test:postgres-adapter`、`npm run build` 等开发命令。
