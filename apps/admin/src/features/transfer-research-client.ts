...
export const loadSyntheticTransferPreview = (apiBase: string, token: string, studyId: string) =>
  request<TransferEvidenceRecordCandidate>(
    apiBase,
    token,
    `/api/v1/bff/admin/transfer-research-designs/${encodeURIComponent(studyId)}/synthetic-preview`
  );
export const reviseTransferResearchDesign = (
  ...
  request<TransferResearchDesignBundle>(
    apiBase,
    token,
    `/api/v1/bff/admin/transfer-research-designs/${encodeURIComponent(studyId)}/revise`,
    {
...
export const retireTransferResearchDesign = (apiBase: string, token: string, studyId: string) =>
  request<TransferResearchDesignBundle>(
    apiBase,
    token,
    `/api/v1/bff/admin/transfer-research-designs/${encodeURIComponent(studyId)}/retire`,
    {
      method: "POST"
    }
  );
