    ...
    >
      ...
      {record ? (
        <>
          ...
          {record.operating_world_consequence ? (
            <article className="candidate-surface" aria-label="Operating World consequence trace teacher">
              ...
              <p>
                分类：{record.operating_world_consequence.source_classification} · Delta：
                {record.operating_world_consequence.official_delta} · Settlement digest：
                {record.operating_world_consequence.replay_relevant_digest}
              </p>
              <p>
                W4 action：{record.operating_world_consequence.w4_action_ref ?? "无"} · Replay manifest：
                {record.operating_world_consequence.w4_replay_manifest_ref ?? "无"}
              </p>
              <ul>
                {record.operating_world_consequence.allowed_effects.map((effect) => (
              ...
              <small>
                仅由确定性系统事实生成；写入官方状态：
                {String(record.operating_world_consequence.writes_official_state)}
              </small>
            </article>
          ) : null}
