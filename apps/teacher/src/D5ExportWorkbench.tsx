...
export function getTeacherD5ErrorMessage(cause: unknown, phase: D5ErrorPhase): string {
...
export function D5ExportWorkbench({
  ...
  return (
    <D5ExportWorkbenchView
      ...
      badge=""
      boundary={
        "教师安全导出。Mock LRS 在进程内运行，不导出学员路由、原始证据、邮件、评分、排名、结算或回放数据。"
      }
      ...
      mapError={(cause) => getTeacherD5ErrorMessage(cause, "operation")}
      mapLoadError={(cause) => getTeacherD5ErrorMessage(cause, "load")}
    />
  );
}
