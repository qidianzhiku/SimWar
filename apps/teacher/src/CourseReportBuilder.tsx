...
export function CourseReportBuilder(props: {
  ...
  async function preview(): Promise<void> {
    ...
    setReceipt(null);
    try {
      const next = await loadTeacherCourseReport(filter, {
  ...
  async function exportReport(format: "json" | "csv"): Promise<void> {
    ...
    setReceipt(null);
    try {
      const next = await exportTeacherCourseReport(filter, format, {
        tenantId: props.tenantId,
        token: props.token
      });
      if (epoch !== requestEpoch.current) return;
  ...
  return (
    <section className="course-report-surface" aria-label="Teacher Course Report Builder">
      ...
      {report ? (
        <article className="candidate-preview" aria-label="Course report preview">
      ...
      {receipt ? (
        <article className="candidate-preview" aria-label="Course report export receipt">
