import type {
  ...
  CourseReportTeacherDto
} from "@simwar/shared-contracts";

export const TEACHER_COURSE_REPORT_PATH = "/api/v1/bff/teacher/course-reports";
export const TEACHER_COURSE_REPORT_EXPORT_PATH = `${TEACHER_COURSE_REPORT_PATH}/export`;

type Fetcher = (input: string, init?: RequestInit) => Promise<Response>;
type ResponseEnvelope<TData> = { code?: unknown; data?: TData };
...
function isSafeReport(value: unknown): value is CourseReportTeacherDto {
...
export function loadTeacherCourseReport(
  filter: CourseReportFilterInput,
  token: string,
  fetcher: Fetcher = fetch
): Promise<CourseReportTeacherDto> {
  return request(`${TEACHER_COURSE_REPORT_PATH}${queryFor(filter)}`, token, fetcher, isSafeReport);
}

export function exportTeacherCourseReport(
  filter: CourseReportFilterInput,
  format: CourseReportExportFormat,
  token: string,
  fetcher: Fetcher = fetch
): Promise<CourseReportExportDto> {
  return request(`${TEACHER_COURSE_REPORT_EXPORT_PATH}${queryFor(filter, format)}`, token, fetcher, (value): value is CourseReportExportDto =>
    isRecord(value) &&
