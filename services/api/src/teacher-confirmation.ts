...
export class TeacherConfirmationCommandService {
  ...
  async saveDraft(
    ...
    requestId: string
  ): Promise<{ data: { confirmation: TeacherConfirmationVersion; status: "generated" | "reused" }; known_limits: readonly string[]; runtime_authority: "JSON_INTERNAL_ONLY" }> {
    identity(actor.actor_id);
    identity(actor.tenant_id);
    identity(requestId);
    this.assertInput(actor.tenant_id, input);
    const refs = [input.course_package_ref, input.learning_goal_ref, input.rubric_ref, ...input.evidence_refs];
    ...
    if (existing) {
      if (existing.content_digest !== contentDigest) throw new TeacherConfirmationError("D3_DUPLICATE_CONFLICT");
      return { data: { confirmation: clone(existing), status: "reused" }, known_limits: [...KNOWN_LIMITS], runtime_authority: "JSON_INTERNAL_ONLY" };
    }
    const confirmation = this.createVersion(actor, input, "DRAFT", "1.0.0", contentDigest, requestId);
    await this.append(confirmation, actor, requestId, "teacher_confirmation.draft_save");
    return { data: { confirmation, status: "generated" }, known_limits: [...KNOWN_LIMITS], runtime_authority: "JSON_INTERNAL_ONLY" };
  }
  ...
  async confirm(
    ...
    if (!latest || latest.status !== "DRAFT") throw new TeacherConfirmationError("D3_LIFECYCLE_INVALID");
    ...
    if (!isTeacherConfirmationVersion(confirmation)) throw new TeacherConfirmationError("D3_OUTPUT_INVALID");
    await this.append(confirmation, actor, requestId, "teacher_confirmation.confirm");
    return { data: { confirmation, known_limits: [...KNOWN_LIMITS], runtime_authority: "JSON_INTERNAL_ONLY" }, known_limits: [...KNOWN_LIMITS] };
  }
  ...
  private async latest(tenantId: string, confirmationId: string): Promise<TeacherConfirmationVersion | null> {
    ...
    return records
      ...
      .sort((left, right) => versionNumber(left.confirmation_ref.version) - versionNumber(right.confirmation_ref.version))
      .at(-1) ?? null;
  }

  private async assertReferences(tenantId: string, input: TeacherConfirmationCommandInput): Promise<void> {
    if (!isTeacherConfirmationExactRef(input.course_package_ref) || input.course_package_ref.resource_type !== "course_package_version") throw new TeacherConfirmationError("D3_EXACT_REF_INVALID");
    const coursePackage = await this.dependencies.coursePackages.getByReference(tenantId, input.course_package_ref);
    if (!coursePackage || coursePackage.status !== "AVAILABLE" || coursePackage.content_digest !== input.course_package_ref.content_digest) throw new TeacherConfirmationError("D3_NOT_FOUND");
    const goal = await this.dependencies.learningDesign.getGoal({ content_digest: input.learning_goal_ref.content_digest, goal_id: input.learning_goal_ref.resource_id, tenant_id: tenantId, version: input.learning_goal_ref.version });
    const rubric = await this.dependencies.learningDesign.getRubric({ content_digest: input.rubric_ref.content_digest, rubric_id: input.rubric_ref.resource_id, tenant_id: tenantId, version: input.rubric_ref.version });
    if (!goal || goal.status !== "PUBLISHED" || goal.content_digest !== input.learning_goal_ref.content_digest) throw new TeacherConfirmationError("D3_NOT_FOUND");
    if (!rubric || rubric.status !== "PUBLISHED" || rubric.content_digest !== input.rubric_ref.content_digest) throw new TeacherConfirmationError("D3_NOT_FOUND");
    if (goal.course_package_reference.course_package_id !== input.course_package_ref.resource_id || rubric.course_package_reference.course_package_id !== input.course_package_ref.resource_id) throw new TeacherConfirmationError("D3_SCOPE_CONFLICT");
    const criterionIds = new Map(rubric.criteria.map((criterion) => [criterion.criterion_id, new Set(criterion.levels.map((level) => level.ordinal))]));
    for (const decision of input.criterion_decisions) {
      const levels = criterionIds.get(decision.criterion_id);
      if (!levels || !levels.has(decision.level_ordinal)) throw new TeacherConfirmationError("D3_RUBRIC_OPTION_INVALID");
    }
    for (const evidenceRef of input.evidence_refs) {
      if (!isTeacherConfirmationExactRef(evidenceRef) || evidenceRef.resource_type !== "evidence_artifact") throw new TeacherConfirmationError("D3_EVIDENCE_INVALID");
  ...
  private createVersion(
    ...
    const confirmation: TeacherConfirmationVersion = {
      ...
      confirmation_ref: { content_digest: contentDigest, discriminator: "exact_ref", resource_id: input.confirmation_id, resource_type: "teacher_confirmation_version", tenant_id: actor.tenant_id, version },
      content_digest: contentDigest,
  ...
  private async append(
    ...
    const auditLog: AuditLog = {
      ...
      after: { confirmation_ref: confirmation.confirmation_ref, status: confirmation.status, content_digest: confirmation.content_digest }
    };
