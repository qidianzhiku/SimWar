...
export class TransferResearchDesignCommandService {
  ...
  async freeze(
    ...
    try {
      const status = await this.repository.appendStudy(bundle.study);
      await this.repository.appendCandidate(bundle.synthetic_preview);
  ...
  private async build(
    ...
    const candidate: TransferEvidenceRecordCandidate = {
      ...
      context_snapshot: { factors: ["OPPORTUNITY_TO_PERFORM"], opportunity_status: "AVAILABLE" },
      created_at: this.now(),
      formal_transfer_claim_write: false,
      instrument_ref: instrumentRef,
