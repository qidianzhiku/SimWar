...
import {
  TransferResearchDesignCommandService,
  TransferResearchDesignError
} from "../transfer-research-design.js";

interface D6RouteContext {
  readonly requestId: string;
  readonly tenantId: string;
}
interface D6RouteRuntime {
  readonly transferResearchDesign: TransferResearchDesignCommandService;
}
...
export function isTransferResearchDesignRoute(method: string | undefined, url: URL): boolean {
  return (
    ...
    /^\/api\/v1\/bff\/(?:teacher|admin)\/transfer-research-designs(?:\/[^/]+\/(?:synthetic-preview|revise|retire)|\/preview|\/freeze)?$/.test(
      url.pathname
    )
  );
}

export async function handleTransferResearchDesignRoute(
  runtime: D6RouteRuntime,
  request: IncomingMessage,
  response: ServerResponse,
  url: URL,
  context: D6RouteContext,
  tools: D6RouteTools
): Promise<boolean> {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === prefix) {
      tools.sendJson(
        response,
        200,
        tools.createEnvelope(context, await runtime.transferResearchDesign.list(context.tenantId))
      );
      return true;
    }
    if (
      request.method === "POST" &&
      (url.pathname === `${prefix}/preview` || url.pathname === `${prefix}/freeze`)
    ) {
      ...
      if (url.pathname.endsWith("/preview")) {
        tools.sendJson(
          response,
          200,
          tools.createEnvelope(
            context,
            await runtime.transferResearchDesign.preview(context.tenantId, input)
          )
        );
      } else {
        const result = await runtime.transferResearchDesign.freeze(
    ...
    const match = new RegExp(
      `^${prefix.replaceAll("/", "\\/")}\\/([^/]+)\\/synthetic-preview$`
    ).exec(url.pathname);
    if (request.method === "GET" && match?.[1]) {
      tools.sendJson(
        ...
        tools.createEnvelope(
          context,
          await runtime.transferResearchDesign.syntheticPreview(
            { actor_id: actor.user_id, tenant_id: context.tenantId },
            match[1]
          )
        )
    ...
    const reviseMatch = new RegExp(`^${prefix.replaceAll("/", "\\/")}\\/([^/]+)\\/revise$`).exec(
      url.pathname
    );
    if (request.method === "POST" && reviseMatch?.[1]) {
      const input = parseInput(await tools.readJson(request, { requiredObject: true }));
      const result = await runtime.transferResearchDesign.revise(
        { actor_id: actor.user_id, tenant_id: context.tenantId },
        reviseMatch[1],
        input
      );
    ...
    const retireMatch = new RegExp(`^${prefix.replaceAll("/", "\\/")}\\/([^/]+)\\/retire$`).exec(
      url.pathname
    );
    if (request.method === "POST" && retireMatch?.[1]) {
      const retired = await runtime.transferResearchDesign.retire(
        { actor_id: actor.user_id, tenant_id: context.tenantId },
        retireMatch[1]
      );
      tools.sendJson(response, 200, tools.createEnvelope(context, retired, "retired"));
      return true;
    }
    return false;
  } catch (error) {
    ...
    tools.sendJson(response, statusFor(error.code), {
      ...
      code: error.code,
      message: "D6 transfer research operation rejected",
      details: []
    });
    return true;
  }
