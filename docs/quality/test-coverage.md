...
当前仓库已将 P1 身份权限底座纳入 `npm test`：

| 测试文件                                 | 覆盖范围                                                                                         |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `tests/unit/health.test.ts`              | 健康契约与真值保护字段基线                                                                       |
| `tests/integration/p0-flow.test.ts`      | 教师登录、Run 创建、开轮、学员提交、锁轮、内部结算、发布、学员视图裁剪、跨租户拒绝               |
| `tests/integration/p1-auth-rbac.test.ts` | 平台管理员创建租户、租户管理员越权拦截、租户内用户创建、session 吊销、审计过滤、真值字段写入拒绝 |

契约检查由 `npm run test:contract` 执行，当前验证 OpenAPI 中的 P0/P1 关键路径和所有 `contracts/schemas/*.v1.json` 可解析。
