...
| [`recovery-state-matrix.csv`](./recovery-state-matrix.csv)               | R3 cross-role recovery, keyboard, reflow, motion and evidence-boundary acceptance matrix                  | shared UI + role roots          |
| [`recovery-convergence.md`](./recovery-convergence.md)                   | R3 shared recovery rail scope and truth-protection boundary                                                | exact source composition        |
...
- route coverage：34 个当前逻辑 hash 目的地（Admin 10、Teacher 12、Student 12）；Enterprise 是 Admin 内的只读 logical projection，不另造 route。
- page inventory coverage：39 个 page inventory surface；共享 `/` 并不合并独立 workbench 的 evidence 责任。
- state matrix coverage：32 个 aggregate route/BFF matrix 行，再按 applicable state/command cell 单独统计；`not applicable` 不进入分母，`source`、`not captured`、`known limit` 与 `not implemented` 不得计为 runtime-observed。

因此 `ROUTE_COVERAGE=100%` 只能表示 34 个当前逻辑目的地均有直接 DOM/browser 证据，不能表示所有状态变体均被观察。DesignSystemLab 证明共享 state component 的视觉与可访问性合同；不得把 DesignSystemLab 的 synthetic state 当作每个业务 workbench 的状态证据。PR4 的 20 张关键页面截图同样是视觉回归集合，不替代 route/BFF/state cell 的功能证据。

`STATE_COVERAGE=PASS` 在本仓库中表示 state contract/classification 完整：32 行 × 15 个 state/command 列共 480 个 cell 均有显式分类，共享 Lab 覆盖 loading/empty/partial/ready/blocked/stale/conflict/unknown/permission-denied/error 视觉合同，且对应业务测试继续保护 directly-observed 路径。它不等于“480 个 cell 都有浏览器截图”。当前冻结分布是：49 `observed*`、325 `source*`、67 `not captured`、11 `known limit`、1 `known contract gaps`、22 `not applicable`、5 `not implemented`；这些限制类与 source-only 证据必须在最终 Known Limits 中原样披露，不能升级成 runtime PASS。
