...
| Generic UI browser lane            | `PASS_WITH_LIMITS`                     | 117 passed, 12 fixture-gated skips. A separate existing role-workflow lane test timed out on a disabled `创建 Run` button; the remote required browser-smoke job passed.                                                                                       |
