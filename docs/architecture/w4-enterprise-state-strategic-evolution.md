...
Student 通过 `/api/v1/bff/student/w4/.../portfolio` 获取脱敏投影，不包含 cash、`state_true`、score、rank 或其他队伍私有字段；Teacher 读取只读的 Opening/Closing、Commitment、Effect、milestone 和 blocker；Admin 的 `/api/v1/bff/admin/w4/portfolio` 提供 Group / Portfolio / Project / Facility 聚合投影。
