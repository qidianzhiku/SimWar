...
按照 V3 规划，康养场景、行业数据和校准属于独立的 Industry Scenario / Industry Data Program，而不是 Core Roadmap。当前 eldercare plugin v1 的上海 `v2` 场景仅使用合成、未校准的教学变量；它可以把护理等级、支付结构、探视便利、资质门槛、政策类教学 shock 和地理摩擦映射为受控候选输入，但不构成真实市场、公共政策或投资决策模型。L1 仍由批准的核心实现承担真值，AI 只提供 advisory-only 的解释与建议。
...
```json
{
  ...
  "state_extensions": ["care_grade_mix", "ltci_eligibility_rate", "visit_friction_index"],
  "settlement_hooks": [
  ...
  "test_cases": ["manifest_schema_validation", "plugin_compatibility", "shadow_replay_baseline"],
...
| 入住客户    | 上海核心区家庭、外圈社区家庭、旅居中产、社区活力老人 | 决定客群迁移与支付曲线         |
...
| 客群迁移     | 上海核心区→上海外圈、本地→异地、活力→照护  | 受价格差、政策与探视便利影响      |
...
上海合成康养教学场景使用版本化的六轮教学脚本：第二轮可引入合成的支付与护理支持情境，第三轮引入城市内通勤与探视便利变化，第四轮引入公共卫生或服务质量事件，第五至第六轮进入整合与复盘。它不声称反映真实上海项目、政策或市场参数。
...
| TASK-PLG-015 | 实现康养插件 v1                | plugins/eldercare   | P0     | 前述核心任务                         | 支持上海合成教学场景与受控教学脚本   |
...
| ADR-PLUGIN-005 | 康养行业作为首个合成插件样板               | 以上海合成教学场景验证双层架构                        | Updated by V3 | 不引入真实数据或校准 |
