...
- `npm run test:e2e:ui:p2b`：独立的 FE-19/FE-20 real-BFF 验收，显式启用 W3 fixture，覆盖 Student 六阶段/反思 201、Teacher 五阶段/本地草稿，以及 blocked、422 error 和恢复；不改变默认 PR4 视觉 fixture。
- `npm run measure:frontend:budgets -- --base-sha <BASE> --head-sha <HEAD>`：读取三端 built dist 的 `index-*` 初始入口 JS/CSS，输出带 BASE/HEAD/actual SHA provenance 的 raw/gzip 初始传输预算；lazy workbench chunks 由 PR4 浏览器/route evidence 单独记录，避免把异步资源重复算入首屏预算。
