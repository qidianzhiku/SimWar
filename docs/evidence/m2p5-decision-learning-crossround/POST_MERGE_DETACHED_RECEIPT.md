...
- The browser fixture is synthetic and pre-seeds the authoritative-looking
  `SettlementResult` fields (`state_true`, score and rank) plus an open second
  round. The result proves the read projection and scope/firewall behavior over
  that fixture, not the upstream workflow that generated settlement or round
  truth.
