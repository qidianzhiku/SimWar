...
import { isStudentLearningReport, isStudentLearningReportListDto } from "@simwar/shared-contracts";

const schema = JSON.parse(
  readFileSync(resolve(process.cwd(), "contracts/schemas/student-learning-report.v1.json"), "utf8")
);
const valid = JSON.parse(
  readFileSync(
    resolve(process.cwd(), "contracts/fixtures/student-learning-report.valid.json"),
    "utf8"
  )
);
const invalid = JSON.parse(
  readFileSync(
    resolve(process.cwd(), "contracts/fixtures/student-learning-report.invalid.json"),
    "utf8"
  )
);
