...
test("Admin previews and exports only the frozen safe report projection", async ({ page }) => {
  const observedRequests: Array<{ headers: Record<string, string>; url: string }> = [];
  await page.route("**/api/v1/bff/admin/course-reports/**", async (route) => {
    const request = route.request();
    observedRequests.push({ headers: request.headers(), url: request.url() });
    const format = new URL(request.url()).searchParams.get("format") as "json" | "csv";
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify(
        envelope({ export_format: format, file_name: `report.${format}`, report })
      )
    });
  });
  await page.route("**/api/v1/bff/admin/course-reports?*", async (route) => {
  ...
  expect(observedRequests.every((request) => new URL(request.url).origin === apiBaseUrl)).toBe(
    true
  );
  expect(
    observedRequests.every((request) => request.headers["x-tenant-id"] === "tenant_demo")
  ).toBe(true);
  expect(
    observedRequests.every((request) => !new URL(request.url).searchParams.has("tenant_id"))
  ).toBe(true);
});

test("Teacher ignores a stale report response after its selected scope changes", async ({
  page
}) => {
  ...
  await page.route("**/api/v1/bff/teacher/course-reports?*", async (route) => {
    expect(route.request().headers()["x-tenant-id"]).toBe("tenant_demo");
    expect(new URL(route.request().url()).origin).toBe(apiBaseUrl);
    calls += 1;
    if (calls === 1) {
      await oldResponse;
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify(
          envelope({ ...report, rows: [{ ...report.rows[0], team_name: "Old Team" }] })
        )
      });
      return;
    }
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify(
        envelope({ ...report, rows: [{ ...report.rows[0], team_name: "Current Team" }] })
      )
    });
  });
